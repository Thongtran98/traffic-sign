...
img_label = {"di thang": 0, "re trai": 1, "re phai": 2, "dung lai": 3, "cam coi": 4, "qua duong": 5, "background": 6}
...
def object_detection_dir(test_dir, write_dir, write_bin_dir, model_path, result_txt):
    ...
    for index, img_name in enumerate(img_names):
        ...
        save_path = os.path.join(write_dir,img_name.split(".")[0]+"_result.png")
        save_bin_path = os.path.join(write_bin_dir,img_name.split(".")[0]+"_bin.png")
        ...
        for rect in rects:
            ...
            x2 = min(height, int(xc+size/2))
            y2 = min(width, int(yc+size/2))
...
if __name__ == "__main__":
    test_dir = "./input"
    write_dir = "./output/output_dir/test_results"
    write_bin_dir = "./output/output_dir/test_results_bin"
    result_txt = "./output/output_dir/test_result.txt"
    object_detection_dir(test_dir, write_dir, write_bin_dir, "./svm_hog_classification/svm_model.pkl", result_txt)
    print ("finished.")
